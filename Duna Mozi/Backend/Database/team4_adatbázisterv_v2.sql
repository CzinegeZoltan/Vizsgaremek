-- Active: 1697197686851@@127.0.0.1@3306@dunamoziEngineering
...
CREATE TABLE IF NOT EXISTS `dunamozi`.`ulesek` (
  ...
  CONSTRAINT `fk_ulesek_vetitoverem1`
    FOREIGN KEY (`vetitoterem_idvetitoverem`)
    REFERENCES `dunamozi`.`vetitoterem` (`idtetitoterem`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
(NULL,"A fegyvertelen katona",NULL,"A Hacksaw Ridge Desmond Doss [Andrew Garfield] szinte hihetetlen története, aki a második világháború egyik legvéresebb csatájában Okinawa szigetén 75 bajtársa életét mentette meg úgy, hogy ő maga nem viselt fegyvert. Vallási meggyőződése volt, hogy noha hazája igazságos háborút vív, neki magának nem szabad ölnie. Szanitécként több alkalommal is egymaga hozta ki sebesült bajtársait az ellenséges vonalak mögül vagy a tűz alatt tartott senkiföldjéről, miközben ő maga is súlyos séruleseket szenvedett. Doss volt az első olyan katona, aki lelkiismereti okokból megtagadta a fegyveres szolgálatot, és megkapta a Kongresszusi Becsület Érdemrendet, Amerika legmagasabb katonai kitüntetését.","131 perc",18,14,"https://upload.wikimedia.org/wikipedia/hu/0/0f/A-fegyvertelen-katona.jpg"),
